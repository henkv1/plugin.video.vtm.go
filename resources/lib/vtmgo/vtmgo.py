...
class VtmGo:
    ...
    def get_detail(self, detail_id, cache=CACHE_AUTO):
        ...
        geoblocked=None
        if detail.get('portraitTeaserOverlayImageUrl'):
            if '242844606' in detail.get('portraitTeaserOverlayImageUrl'):
                geoblocked=True
    ...
    def _parse_teaser(self, item, cache=CACHE_ONLY):
        ...
        if item.get('overlayImageUrl'):
            ...
            if '248317222' in item.get('overlayImageUrl'):
                vtmgoplus=True
            if '247901632' in item.get('overlayImageUrl'):
                vtmgoplus=True
            if '247901658' in item.get('overlayImageUrl'):
                vtmgoplus=True
            if '248317223' in item.get('overlayImageUrl'):
                vtmgoplus=True
    ...
    def _parse_episode_teaser(self, item, cache=CACHE_ONLY):
        ...
        geoblocked=None
        if item.get('target', {}).get('overlayImageUrl'):
            if '242844604' in item.get('target', {}).get('overlayImageUrl'):
                geoblocked=True
